  ...
  describe("Properties", () => {
    ...
    describe("Method", () => {
      it("should send a GET request.", () => {
        expect(createTestTarget().get().build().method).toEqual(
          ZHttpMethod.Get,
        );
      });

      it("should send a POST request.", () => {
        expect(createTestTarget().post().build().method).toEqual(
          ZHttpMethod.Post,
        );
      });
      ...
      it("should send a POST request with a body.", () => {
        ...
        expect(createTestTarget().post(expected).build().body).toEqual(
          expected,
        );
      });

      it("should send a PUT request.", () => {
        expect(createTestTarget().put().build().method).toEqual(
          ZHttpMethod.Put,
        );
      });
      ...
      it("should send a DELETE request.", () => {
        expect(createTestTarget().delete().build().method).toEqual(
          ZHttpMethod.Delete,
        );
      });

      it("should send a PATCH request.", () => {
        expect(createTestTarget().patch().build().method).toEqual(
          ZHttpMethod.Patch,
        );
      });
      ...
      it("should send a PATCH request with a body.", () => {
        const expected = { value: 0 };
        expect(createTestTarget().patch(expected).build().body).toEqual(
          expected,
        );
      });

      it("should send a OPTIONS request.", () => {
        expect(createTestTarget().options().build().method).toEqual(
          ZHttpMethod.Options,
        );
      });

      it("should send a HEAD request.", () => {
        expect(createTestTarget().head().build().method).toEqual(
          ZHttpMethod.Head,
        );
      });
    });
    ...
    describe("Headers", () => {
      it("should add all headers.", () => {
        ...
        expect(createTestTarget().headers(expected).build().headers).toEqual(
          expected,
        );
      });
