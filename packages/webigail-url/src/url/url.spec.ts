...
describe("ZUrlBuilder", () => {
  ...
  describe("Parts", () => {
    ...
    it("cleans the protocol.", () => {
      ...
      expect(createTestTarget().protocol(`${protocol}://`).build()).toEqual(
        expected,
      );
    });
    ...
    it("sets the correct host.", () => {
      ...
      expect(createTestTarget().hostname("www.yahoo.com").build()).toEqual(
        expected,
      );
    });
    ...
    it("cleans the host.", () => {
      const expected = `${protocol}://${hostname}`;
      expect(
        createTestTarget().hostname(`////////${hostname}////`).build(),
      ).toEqual(expected);
    });

    it("sets the correct path.", () => {
      const expected = `${protocol}://${hostname}/a/b/c/d`;
      expect(createTestTarget().append("a/b").append("/c/d/").build()).toEqual(
        expected,
      );
    });
    ...
    it("sets the hash.", () => {
      ...
      expect(createTestTarget().append("/a/b").hash("abcd").build()).toEqual(
        expected,
      );
    });
    ...
    it("ignores port 0.", () => {
      ...
      expect(createTestTarget().protocol("http").port(0).build()).toEqual(
        expected,
      );
    });
    ...
    it("ignores default ports.", () => {
      const expected = `http://${hostname}`;
      expect(createTestTarget().protocol("http").port(80).build()).toEqual(
        expected,
      );
    });
    ...
    it("adds the search.", () => {
      ...
      expect(
        createTestTarget().param("paramA", "a").param("paramB", "b").build(),
      ).toEqual(expected);
    });
    ...
    it("sets the domain as the host if the host is not set.", () => {
      ...
      expect(createTestTarget().hostname("").subdomain("mail").build()).toEqual(
        expected,
      );
    });
    ...
    it("encodes the parameter values.", () => {
      ...
      expect(createTestTarget().param("paramA", valA).build()).toEqual(
        expected,
      );
    });
    ...
    it("sets the password.", () => {
      ...
      expect(
        createTestTarget().username("user").password("password").build(),
      ).toEqual(expected);
    });
  ...
  describe("Parsing", () => {
    it("correctly parses a url.", () => {
      const expected =
        "https://user:password@www.google.com:9086/foo/bar/?valA=a&valB=b#hhh";
    ...
    it("can be modified.", () => {
      ...
      const expected =
        "http://user:password@mail.google.com:9099/a/b/c/?valA=c&valB=d#h32";
      expect(
        createTestTarget()
          .parse(url)
          .protocol("http")
    ...
    it("can be modified to use a different port.", () => {
      ...
      expect(createTestTarget().parse(url).port(9999).build()).toEqual(
        expected,
      );
    });
  });
  ...
  describe("Location", () => {
    ...
    it("constructs the search params.", () => {
      // Arrange
      const target = createTestTarget();
      loc.search = "?a=b";
      // Act
  ...
  describe("Gravatar", () => {
    it("returns the base gravatar url.", () => {
      expect(createTestTarget().gravatar().build()).toEqual(
        ZUrlBuilder.UrlGravatar,
      );
    });
  ...
  describe("YouTube", () => {
    it("returns the base YouTube url.", () => {
      expect(createTestTarget().youTube().build()).toEqual(
        ZUrlBuilder.UrlYouTube,
      );
    });
    ...
    it("returns an watch video url for YouTube.", () => {
      ...
      const expected = createTestTarget()
        .parse(ZUrlBuilder.UrlYouTube)
        .append("watch")
        .param("v", id)
        .build();
      expect(createTestTarget().youTube(ZYouTubeApi.Watch, id).build()).toEqual(
        expected,
      );
    });
    ...
    it("returns an embed video url for YouTube.", () => {
      ...
      const expected = createTestTarget()
        ...
        .append(id)
        .build();
      expect(createTestTarget().youTube(ZYouTubeApi.Embed, id).build()).toEqual(
        expected,
      );
    });
  });
  ...
  describe("Query", () => {
    const shouldAddParam = (
      expected: string,
      key: string,
      setFn: (t: ZUrlBuilder) => ZUrlBuilder,
    ) => {
      // Arrange.
      const target = createTestTarget();
      // Act.
