...
describe("ZRestfulService", () => {
  ...
  describe("Count", () => {
    it("should return the total number of items across all pages", async () => {
      ...
      const filter = new ZFilterLogicBuilder()
        ...
        .clause(new ZFilterUnaryBuilder().isNotNull().subject("id").build())
        .build();
      const expected = new ZPageBuilder<any>()
        .data([fire])
        .count(types.length)
        .build();
      http.set(
      ...
      const actual = await target.count(
        new ZDataRequestBuilder().search("fir").filter(filter).build(),
      );
      // Assert.
  ...
  describe("Create", () => {
    it("should attempt to create a resource", async () => {
      ...
      http.set(
        target.endpoint().build(),
        ZHttpMethod.Post,
        new ZHttpResultBuilder(dragon).build(),
      );
      // Act.
  ...
  describe("Update", () => {
    it("should attempt to update a resource", async () => {
      ...
      const target = createTestTarget();
      http.set(
        target.endpoint(fire.id).build(),
        ZHttpMethod.Patch,
        new ZHttpResultBuilder(fire).build(),
      );
      // Act.
  ...
  describe("Delete", () => {
    it("should attempt to delete a resource", async () => {
      ...
      http.set(
        url,
        ZHttpMethod.Delete,
        new ZHttpResultBuilder(undefined)
